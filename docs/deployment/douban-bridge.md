...
当前 `apps/core/wrangler.jsonc` 的 **vars**（明文配置，不是 secret）：`DOUBAN_API_KEY`、`TRAKT_CLIENT_ID`、`GITHUB_CLIENT_ID`、`STREMIO_ORIGIN`、`DASH_ORIGIN`、`AGENT_MATCH_MODEL`、`AGENT_MATCH_GATEWAY_ID`、`AGENT_MATCH_GATEWAY_PROVIDER`。core 还有 Workers AI binding `AI`（走 AI Gateway 自定义 provider，不需要 `AGENT_MATCH_API_KEY`）。代码还读取以下名称，它们应作为 **secrets**（或尚未配置）：`JWT_SECRET`、`GITHUB_CLIENT_SECRET`、`TMDB_API_KEY`、`EXA_API_KEY`；可选 `FANART_API_KEY` / `TRAKT_CLIENT_SECRET`。获得授权后用下面命令核对实际存在的名称，**不要把值写入仓库或本文**：
